...
class ChiqimsavdoControllerRu extends Controller
{
    //
     //add Kirim
     public function chiqimbugunson()
     {
        $user = auth()->user();
         ...
         return view('russian.chiqim_bugun_son', compact('olchams', 'tovars', 'users', 'chiqim'));
     }
     ...
     $users = User::where('firmaid', $userId)->orwhere('id', $userId)->get();
         $chiqim = Chiqimsavdo::where('firmaid', $userId)->where('sotildi', 1)->whereDate('created_at', $today)->get();
         return view('russian.chiqim_bugun', compact('olchams', 'tovars', 'users', 'chiqim'));
     }
     ...
     {  
        ...
        $kirims = Kirim::where('firmaid', $userId)->where('muddati', '>', now())->orderBy('muddati', 'desc')->get();
         ...
         return view('russian.chiqim', compact('kirims', 'olchams', 'tovars', 'chiqims', 'chiqim'));
     }

     public function chiqimsana(){
        return view('russian.chiqimsana');        
     }

     public function chiqimsanason(){
        return view('russian.chiqimsana_son');        
     }
     ...
     public function sotildi()
     {
        ...
        if($user->type == 'admin'){
         ...
         return redirect()->route('adminchiqimru')->with('success', 'Продано.');
     }
     ...
     public function delete(Request $request, $id)
     {
         ...
         // Redirect back with a success message
         return redirect()->route('adminchiqimru')->with('success', 'Удалено');
         }
     ...
     public function chiqimsanaqidir(Request $request)
     {
        ...
        ->get();
         ...
         $olchams = Olchamlar::where('firmaid', $userId)->get();
         $users = User::where('firmaid', $userId)->orwhere('id', $userId)->get();
         return view('russian.chiqimsanashow', compact('users', 'olchams', 'tovars', 'chiqim'));
     }
     ...
     public function chiqimsanaqidirson(Request $request)
     {
        ...
        ->get();
         ...
         $olchams = Olchamlar::where('firmaid', $userId)->get();
         $users = User::where('firmaid', $userId)->orwhere('id', $userId)->get();
         return view('russian.chiqimsanashowson', compact('users', 'olchams', 'tovars', 'chiqim'));
     }
     ...
     public function qaytar($id)
     {
         ...
         // Redirect back with a success message
         return redirect()->route('adminchiqimru')->with('success', 'Сохранено успешно..');    
     }
}
