                ...
                if (response) {
                    ...
                    try {
                        ...
                        window.setTimeout(function() {
                            window.location.href = "{{ route('adminchiqimru') }}";
                        }, 10);
                    } catch (e) {
...
<br/>
    <div class="row" style="margin: 0.01%;">
            ...
            <div class="card-body">
                <form class="form-horizontal" action="{{ route('chiqim.storeru') }}" name="material" id="material" method="post">
                    @csrf <!-- CSRF token -->
        ...
        <select class="form-control select2 tovar" id="tovar" style="width: 100%">
            ...
            @endforeach 
                    ...
                    <div class="form-group display" style="display: none">
                        <label class="control-label">Товарное наименование:</label>
                        <div class="col-sm-9">
                    ...
                    <div class="form-group display2" style="display: none">
                        <label class="control-label">Цена продажи (за единицу):</label>
                        <div class="col-sm-9">
                    ...
                    <div class="form-group display" style="display: none">
                        <label class="control-label">Количество в наличии:</label>
                        <div class="col-sm-9">
                    ...
                    <div class="form-group display2" style="display: none; ">
                        <label class="control-label" >Количество (штук):</label>
                        <div class="col-sm-9">
    ...
    <tbody>
        ...
        @foreach ($chiqim as $chiq)
            ...
            <tr class="align-middle">
                ...
                <td>
                    <a href="{{ route('chiqim.deleteru', $chiq->id) }}" 
                       class="text-danger" 
                    ...
                    <form id="delete-about-{{ $chiq->id }}" 
                          action="{{ route('chiqim.deleteru', $chiq->id) }}" 
                          method="POST" 
                          style="display: none;">
                        @csrf
                        @method('DELETE')
...
<form action="{{ route('chiqim.sotildiru') }}" method="POST" id="sotildiForm">
    @csrf
    <button type="submit" style="margin-right: 1%; width: 100%; color: white" class="btn btn-success" onclick="printReceipt()">Продано</button>
</form>
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.tovarid > 0) {
                    ...
                    alert('Этот товар по указанному штрих-коду отсутствует.');
                    $('.display').hide();
                    $('.display2').hide();
                    $('#tovarid').val('');
                    $('#tnomi').val('');
        ...
        $.ajax({
            ...
            success: function(data) {
                if (Miqd <= data.ombor && Miqd >= 0) {
                    ...
                    alert("Нельзя ввести количество больше, чем есть в наличии!");
                    $('#Summa').val(0);
                    $('#Summadona').val(0);
                    $('#Miqdori').val(0);
                    $('#Summat').val(0);
...
<script>
    ...
    function printReceipt() {
        ...
        printContent += "<thead  style='font-size: 10px'><tr><th>Название товара</th><th>Количество</th><th>Сумма</th></tr></thead>";
        printContent += "<tbody>";
        ...
        printContent += "<p>Итого: {{ $umumiy }} </p>";
        printContent += "___________________________";
        printContent += "<p>Спасибо за покупку!</p>";
