                ...
                success: function(response) {
                    try {
                        ...
                        window.setTimeout(function() {
                            window.location.href = "{{ route('adminkirimru') }}";
                        }, 4000);
                    } catch (e) {
    ...
    <div class="card mb-4">
        ...
        <form class="form-horizontal" action="{{ route('kirim.storescanru') }} " name="material" id="material" method="post">
            ...
            <div class="form-group display" >
                    ...
                    <div class="col-sm-8">
                        <select name="tovarid" id="tovarid" class="form-control select2" required>
                            ...
                            @foreach ($tovars as $tovar)
                                <option value="{{ $tovar->id }}">{{ $tovar->nomi }}(Цена: {{$tovar->sotilgannarx}})</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="form-group display" style="display: none">
                    <label class=" control-label">Сколько товаров поступило:</label>
                    <div class="col-sm-8">
                ...
                <div class="form-group display2" style="display: none">
                    <label class=" control-label">Сколько товаров (единиц) поступило:</label>
                    <div class="col-sm-8">
                    ...
                    <tbody>
                        @foreach ($tovars as $tovar)
                            ...
                            <tr class="align-middle">
                                ...
                                <td>
                                    <a href="{{ route('kirim.deleteru', $kirim->id) }}" 
                                       class="text-danger" 
                                    ...
                                    <form id="delete-about-{{ $kirim->id }}" 
                                          action="{{ route('kirim.deleteru', $kirim->id) }}" 
                                          method="POST" 
                                          style="display: none;">
                                        @csrf
                                        @method('DELETE')
